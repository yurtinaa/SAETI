...
class SANNI:
    ...
    def __post_init__(self):
        ...
        log_func(f'model: {self.model_predictor}')
        ...
        # log_func(self.pred_func)
        # log_func(self.snippet_size, self.add_nan, self.model_predictor)
    ...
    def preprocess_data_to_classifier(self, train):
        ...
        # model_log_func(self.classifier)
    ...
    def __init_sanni_predictor(self, backward=False):
    ...
    def preprocess_data_to_predictor(self, train, valid):
        ...
        if self.all_snippet is not None:
            ...
            # model_log_func(self.all_snippet.shape)
        if self.model_predictor == 'saeti':
            ...
            predictor = self.__init_sanni_predictor()

            predictor.with_h = self.with_h
            backward_predictor = None
            if self.serial:
                if self.backward:
                    backward_predictor = self.__init_sanni_predictor(backward=True)
                ...
                # model_log_func(self.model_predictor)

        self.predictor = self.predictor.to(self.device)

        # log_func(self.predictor)
    ...
    def fit(self, train, valid):
        model_log_func(f'hidden: {self.hidden}')
        model_log_func('старт обучения')
        self.preprocess_data_to_classifier(train)
        model_log_func('предобработка данных')

        self.train_classifier()
        model_log_func('обучил классификатора')
        self.preprocess_data_to_predictor(train, valid)
        #   print('with_h: ', self.predictor.forward_predictor.with_h)
        model_log_func('обучаю предсказателя')
    ...
    def impute(self, x):
        ...
        miss[:, :] = predict[:, :]
        # print(np.isnan(miss.detach().cpu().numpy()).any())
        return miss.detach().cpu().numpy()
